...
public class BookingServiceImpl implements BookingService {
    ...
    public Boolean deleteByUser(Long id, Principal principal) {
        bookingRepository.deleteById(bookingRepository.findByIdAndUser_Username(id, principal.getName()).orElseThrow(() -> new NoRecordFoundException(messages.get("booking.record.not.found"))).getId());
        return true;
    }

    @Override
    public BookingRequest findById(Long id, Principal principal) {
        return bookingRequestMapper.toBookingRequest(bookingRepository.findByIdAndUser_Username(id, principal.getName()).orElseThrow(() -> new NoRecordFoundException("Booking Record not found")));
    }

    @Override
    public List<BookingRequest> findAll() {
        return bookingRepository.findAll().stream().map(x -> bookingRequestMapper.toBookingRequest(x)).collect(Collectors.toList());
    }
